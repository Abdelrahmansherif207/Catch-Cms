...
export async function fetchAssignments(
  couponId: number,
  { page = 1, limit = 15 }: FetchAssignmentsParams = {}
): Promise<AssignmentListResponse> {
  const params = new URLSearchParams();
  params.append('page', page.toString());
  params.append('limit', limit.toString());
  const { data } = await axiosClient.get<AssignmentListResponse>(
    '/coupons/' + couponId + '/assignments?' + params.toString()
  );
  return data;
}
...
): Promise<ApiResponse<CouponAssignment>> {
  const { data } = await axiosClient.post<ApiResponse<CouponAssignment>>(
    '/coupons/' + couponId + '/assignments',
    payload
  );
  return data;
}
...
export async function updateAssignment(
  ...
  const { data } = await axiosClient.put<ApiResponse<CouponAssignment>>(
