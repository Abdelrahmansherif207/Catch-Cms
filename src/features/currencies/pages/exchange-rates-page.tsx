...
export function ExchangeRatesPage() {
  ...
  const { data: ratesData, isLoading: ratesLoading, isError: ratesError, refetch: refetchRates } = useExchangeRates({
  ...
  return (
    <div className="space-y-4">
      ...
      >
        <div className="flex flex-wrap items-center gap-2">
          <FilterSelect
            ...
            prefix={t('currencyRates.currency')}
            allLabel={t('exchangeRates.allCurrencies')}
            options={currencies.map((currency) => ({
              value: currency.id.toString(),
              label: `${currency.code} — ${typeof currency.name === 'object' ? (currency.name as { en: string; ar: string }).en : currency.name}`,
            }))}
            allValue=""
      ...
      {ratesError && (
        <DataErrorState onRetry={() => refetchRates()} />
      )}
